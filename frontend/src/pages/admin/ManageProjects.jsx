...
const floorOptions = ["G+1", "G+2", "G+3", "G+4", "G+5"];
const plotOptions = [
  "30x40 sq. ft",
  "30x50 sq. ft",
  "40x50 sq. ft",
...
const ManageProjects = () => {
  ...
  useEffect(() => {
    ...
    if (statusFilter !== "all") {
      ...
      filtered = filtered.filter(
        (project) => (project.status || "").toLowerCase() === normalizedStatus.toLowerCase()
      );
    }
  ...
  const buildPayload = (form, extra = {}) => ({
    ...
    completionPercentage: form.completionPercentage ? Number(form.completionPercentage) : 0,
    startDate: form.startDate || undefined,
    endDate: form.endDate || undefined,
    floor: form.floor || undefined,
    plotDimension: form.plotDimension || undefined,
  ...
  const openEditModal = (project) => {
    ...
    setEditForm({
      ...
      completionPercentage:
        project.completionPercentage != null ? String(project.completionPercentage) : "0",
      ...
      floor: project.floor || project.floors || "G+1",
      plotDimension: project.plotDimension || project.plot_dimension || "30x40 sq. ft",
      facing: project.facing || project.direction || "",
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      setFilteredProjects((prev) => prev.map((p) => (p._id === updated._id ? updated : p)));
      setShowEditModal(false);
    } catch (error) {
      setEditError(error?.response?.data?.message || "Failed to update project. Please try again.");
    } finally {
  ...
  const confirmDelete = async () => {
    try {
      ...
      setFilteredProjects((prev) => prev.filter((project) => project._id !== selectedProject._id));
  ...
  const updateStatus = async (project, newStatus) => {
    try {
      const updatedProject = await ProjectService.updateProject(project._id, { status: newStatus });
      setProjects((prev) => prev.map((p) => (p._id === project._id ? updatedProject : p)));
      setFilteredProjects((prev) => prev.map((p) => (p._id === project._id ? updatedProject : p)));
    } catch (error) {
  ...
  const renderCommonFields = (form, onChange, mode) => (
    <>
      ...
      {mode === "create" ? (
        <div>
          <label className="text-sm font-medium text-gray-700">Project Images (max 10)</label>
          <input
            type="file"
            multiple
            accept="image/*"
            onChange={(e) => setCreateImages(Array.from(e.target.files || []).slice(0, 10))}
        ...
        <>
              ...
              <div className="grid grid-cols-3 gap-3">
                {editKeepImages.map((img) => (
                  <div key={img.public_id || img.url} className="relative">
                    <img src={img.url || img} alt="Project" className="w-full h-24 object-cover rounded-lg" />
                    <button
                      type="button"
                      onClick={() =>
                        setEditKeepImages((prev) =>
                          prev.filter((i) => (i.public_id || i.url) !== (img.public_id || img.url))
                        )
                      }
                      className="absolute top-1 right-1 bg-black/60 text-white text-xs px-2 py-1 rounded"
                    >
          ...
          <div>
            <label className="text-sm font-medium text-gray-700">Add/Replace Images (max 10 total)</label>
            <input
              type="file"
              multiple
              accept="image/*"
  ...
  return (
    <main className="bg-white w-full overflow-x-hidden">
      <section className="py-10 px-6 md:px-12 lg:px-16 border-b border-gray-100">
        <span className="text-xs uppercase tracking-[0.35em] text-red-600 font-semibold">Projects</span>
        <h1 className="text-3xl md:text-5xl font-light text-gray-900 mt-4 leading-tight">Manage Projects</h1>
        <p className="mt-4 text-gray-600">View, create, and manage all construction projects.</p>
        <div className="mt-6">
          <Button
            onClick={() => {
              setCreateError("");
              ...
              <tbody>
                {filteredProjects.length === 0 ? (
                  ...
                  filteredProjects.map((project) => (
                    <tr key={project._id} className="border-b last:border-none hover:bg-gray-50 transition">
                      <td className="py-3 px-4 font-medium text-gray-800">{project.title}</td>
                      <td className="py-3 px-4 text-gray-600">{project.client?.name || "-"}</td>
                      <td className="py-3 px-4 text-gray-600">{project.location?.city || project.location?.address || "-"}</td>
                      ...
                      <td className="py-3 px-4 text-gray-600">{project.budget != null ? `INR ${project.budget.toLocaleString()}` : "-"}</td>
                      <td className="py-3 px-4 text-gray-600">{project.startDate ? new Date(project.startDate).toLocaleDateString() : "-"}</td>
                      <td className="py-3 px-4 text-gray-600">{project.endDate ? new Date(project.endDate).toLocaleDateString() : "-"}</td>
                      <td className="py-3 px-4 text-right space-x-2">
                        <Button size="sm" variant="ghost" onClick={() => openDetailsModal(project)}>
                        ...
                        {project.status === "ongoing" && (
                          <Button size="sm" variant="ghost" onClick={() => updateStatus(project, "on_hold")}>
                            <PauseCircle className="w-4 h-4 text-yellow-600" />
                          </Button>
                        )}
                        {project.status === "on_hold" && (
                          <Button size="sm" variant="ghost" onClick={() => updateStatus(project, "ongoing")}>
                            <Clock className="w-4 h-4 text-blue-600" />
                          </Button>
                        )}
                        {project.status !== "completed" && (
                          <Button size="sm" variant="ghost" onClick={() => updateStatus(project, "completed")}>
                            <CheckCircle className="w-4 h-4 text-green-600" />
                          </Button>
                        )}
                        <Button size="sm" variant="danger" onClick={() => handleDeleteClick(project)}>
      ...
      <Modal isOpen={showDetailsModal} onClose={() => setShowDetailsModal(false)} title="Project Details">
        {selectedProject && (
          <div className="space-y-3 text-sm text-gray-700">
            <p><strong>Title:</strong> {selectedProject.title}</p>
            <p><strong>Client:</strong> {selectedProject.client?.name || "-"}</p>
            <p><strong>Location:</strong> {selectedProject.location?.city || selectedProject.location?.address || "-"}</p>
            <p><strong>Status:</strong> {selectedProject.status}</p>
            <p><strong>Budget:</strong> {selectedProject.budget != null ? `INR ${selectedProject.budget.toLocaleString()}` : "-"}</p>
            <p><strong>Floor:</strong> {selectedProject.floor || "-"}</p>
            <p><strong>Plot dimension:</strong> {selectedProject.plotDimension || "-"}</p>
            <p><strong>Facing:</strong> {selectedProject.facing || "-"}</p>
            <p><strong>Budget range:</strong> {selectedProject.budgetRange || "-"}</p>
            <p><strong>Images:</strong> {selectedProject.images?.length || 0}</p>
            <p><strong>Description:</strong> {selectedProject.description}</p>
          </div>
        )}
      </Modal>

      <Modal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} title="Add Project">
        <form className="space-y-4" onSubmit={handleCreateSubmit}>
          {createError && <div className="bg-red-50 text-red-600 text-sm px-4 py-2 rounded-lg">{createError}</div>}
          {renderCommonFields(createForm, handleCreateChange, "create")}
          <div className="flex justify-end gap-3 pt-2">
            <Button type="button" variant="outline" onClick={() => setShowCreateModal(false)}>Cancel</Button>
            <Button type="submit" loading={createLoading} disabled={createLoading}>Create Project</Button>
          </div>
        </form>
      </Modal>
      ...
      <Modal isOpen={showEditModal} onClose={() => setShowEditModal(false)} title="Edit Project">
        <form className="space-y-4" onSubmit={handleEditSubmit}>
          {editError && <div className="bg-red-50 text-red-600 text-sm px-4 py-2 rounded-lg">{editError}</div>}
          {renderCommonFields(editForm, handleEditChange, "edit")}
          <div className="flex justify-end gap-3 pt-2">
            <Button type="button" variant="outline" onClick={() => setShowEditModal(false)}>Cancel</Button>
            <Button type="submit" loading={editLoading} disabled={editLoading}>Update Project</Button>
          </div>
        </form>
      </Modal>
